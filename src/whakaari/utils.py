...
def load_dataframe(
    ...
    if filename.endswith('.csv'):
        ...
        return pd.read_csv(filename, index_col=index_col, parse_dates=parse_dates, usecols=usecols,
                           infer_datetime_format=infer_datetime_format, nrows=nrows, header=header, skiprows=skiprows)
...
def save_dataframe(df: pd.DataFrame, filename, index: bool = True, index_label: str = None) -> bool:
    if filename.endswith('.csv'):
...
def wrapped_indices(max_idx: int, asymmetry_factor: float, sub_domain_range: int, n: float | int) -> List[int]:
...
def compute_rsam(data: List,
    ...
    for _data, band_name in zip(data, band_names):
        ...
        for k, outlier, max_idx in zip(range(m), outliers, max_idxs):
            ...
            if outlier:
                _indices = wrapped_indices(max_idx, asymmetry_factor, sub_domain_range, n)
...
def compute_dsar(data_is: list,
                 ...
                 max_idxs: List[int],
                 asymmetry_factor: float,
                 sub_domain_range: int, ) -> tuple[list, list]:
