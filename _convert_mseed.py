...
def get_data_for_day(i,t0,station):
    ...
    network = 'AV' # Tania: change this as appropriate for directory naming

    pad_f=0.2
    sts = []
    root = r'F:\Data_Poas\SDS' #Tania: hard-code data directory
    network = 'CR'
    channel = 'HHZ.D'
    for k in [-1,0,1]:
        year,day=(t0+(i+k)*_DAY).format_seed().split(',')
        ### @David had to change this
        # nsy = '{:s}/{:s}/{:s}/'.format(network, station, year)
        nsy = '\\{:s}\\{:s}\\{:s}\\{:s}\\'.format(year, network, station, channel)
        ...
        try:
            st = read(root+nsy+'{:s}.{:s}..{:s}.{:s}.{:s}.MSEED'.format(network,station,channel,year,day))
            # CR.CRPO..HHZ.D.2015.002.MSEED
            print(st)
            ok = 'HH*'
            st= st.select(channel=ok)
            sts.append(st)
        except:
            pass
    try:
        st = deepcopy(sts[0]+sts[1]+sts[2])
        # Tania: do your data have site sensitivity removed?
        ...
        # site = client.get_stations(starttime=t0+i*_DAY, endtime=t0+(i+1)*_DAY, station='TBTN',
...
if __name__=="__main__":
    td = TremorDataCo(station='EPDN')  #Tania: change station name as appropriate
    ti = '2014-01-01' #Tania: change this to date range you have available
    tf = '2018-01-01'
